# M1 — live question→answer spine — REVIEWED

Closed record. The roadmap keeps a summary; everything the summary omits is here.
...
- **u2 — Prolog engine worker** · kernel · est 120K · DONE
  ...
  `main=93% 224K/240K`, `mate=69% 167K/240K` (map-m1u2). P6.2's dev-server and
  built-output browser smokes were closed at M1 review by `pnpm browser:check`, which
  drives both modes to 337 documents. NOT verified: the `test-m1u2` red suite, in
...
- **u3 — budgets, failure modes, cancellation** · kernel · est 110K · DONE
  ...
  No unit gauge was recorded for u3; it is unavailable, not zero.
...
- u1 `.agent/contracts/m1u1.md` | `.agent/contracts/m1u1-rev-checkset.md`, 30 rows | `wt/spike-m1u1-det`; partial reports `map-m1u1` 17/25, `spike-m1u1-det` 9/12
...
- u7 `.agent/contracts/m1u7.md` (34 predicates + verdict table + D1–D9) | `.agent/contracts/m1u7-rev-checkset.md`, 26 rows | `wt/map-m1u7` `124e34d` (probe), `wt/res-m1u7` `d712fa9`
- u6 `.agent/contracts/m1u6.md` (32 predicates + verdict table + D1–D10 + Q1–Q8 rulings) | `.agent/contracts/m1u6-rev-checkset.md`, 28 rows | `wt/test-m1u6` `2f87e0b` 23/23, `wt/spike-m1u6-{handle,signal}` `b896d2e`/`3066404`

Projection was 6 `kernel` units × ~35 rows ≈ 210 rows plus u7, cross-cutting and
`audit-m1`, sized at 3–4 sessions. Actual: **354 rows over 6 sessions** — u1 30, u2 28,
u3 45, u4 24, u5 28, u6 28, u7 26, cross-cutting 24, `audit-m1` 137. The projection
counted rows but not the fix backlog each session inherits, which is what doubled the
session count. Yield with seeded skeletons: 44 rows + 5 fixes (session 3, two reviewers),
56 rows + 12 fixes (session 4, two), 28 rows + 16 fixes (session 5, one), 50 rows + 26
fixes (session 6, two). Sizing lesson for M2: a review session's real unit is
`rows + inherited fixes`, and the fix half grows as the row half lands.

Judgment-review ledger = `.agent/archive/review-m1.md`; reviewer reports = `.agent/review-m1/`.
Cross-cutting check set = `.agent/contracts/m1-cross-rev-checkset.md`, 24 rows; `audit-m1`
self-enumerated 137 rows into `.agent/review-m1/audit-m1.ids`.

Session 6 adjudicated **50 rows** — u7 26/26 and cross-cutting 24/24 — which closed the
milestone at 354. It closed **26 defects**: the 3 inherited (R35, R40, E26) plus u7's 7 and
cross-cutting's 16, of which 6 stay accepted-open in `.agent/polish.md` (X02, X03, X14, X19,
X20, X24). Three gate steps entered the chain across the run's last two commits —
`engine:check` at `8ecbbde`, `presentation:check` at `bc3576f` — taking `pnpm gate` to
eleven steps, and `pnpm browser:check` was created at `ef18b3f` for the two claims a gate
step cannot reach. `.scratch/verify-fixes.py` grew 31 → 45 mutants, 45/45 RED.
Session finding: `rev-m1-cross-1` stopped at 8/24 reporting a 178K/240K context limit while
`context-gauge` measured 96K. Reviving it in place with the measured figure recovered the
other 16 rows in the same window; a self-reported context limit is a belief, and the
machine reading is what should decide a handoff. Cost: `main=` 96% at close,
`mate=85% 204K/240K` (rev-m1-cross-1), `mate=61% 147K/240K` (rev-m1u7-1).

The session-5 paragraph below is a historical snapshot. Its 31-mutant, 361-file and
221-test figures were superseded within session 6.
