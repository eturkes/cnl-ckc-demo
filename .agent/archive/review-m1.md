...
**45/45 RED**. Contract-ruling closures carry no mutant: R28, R37, R41, R44, R45, and U04/U08
...
Session 6 adds the documentation and register closures — U7-05, U7-23, U7-24, U7-25 and
X13 through X24 — which are text, configuration or dependency changes with no behaviour
to mutate; each names its own rerunnable check instead.
...
| u7 | docs | `.agent/contracts/m1u7-rev-checkset.md` | 26 | **26 complete** | 6 |
| cross-cutting | — | `.agent/contracts/m1-cross-rev-checkset.md` | 24 | **24 complete** | 6 |
| audit-m1 | — | `.agent/review-m1/audit-m1.ids`, self-enumerated | 137 | 137 | 1 |

**M1 review is COMPLETE at 354 rows** — u1 30, u2 28, u3 45, u4 24, u5 28, u6 28, u7 26,
cross-cutting 24, `audit-m1` 137. There is no resume point; the milestone is REVIEWED.
Six rows stay accepted-but-open and live in `.agent/polish.md` with their acceptance
checks: X02, X03, X14, X19, X20, X24. Evidence branches: `wt/rev-m1u1-1` `939de23` (u1
...
| u5 | rev-m1u5-1 | I28 | fixed(low) | `.agent/contracts/m1u5.md` Source, `.claude/rules/ui.md` bits-ui bullet, suite comments | the dead `.scratch/agents/map-m1u5.md` and `wt/spike-m1u5-lib` `dca4f87` pointers are gone while D2 keeps the measurements, the memory bullet keeps the dependency, DOM and 10.06 s facts, and the suite copy closes with I03 — judgment-only, so the five-file ledger reruns as its check |
| u7 | rev-m1u7-1 | U7-01 | pass | report U7-01; three exact 5.3.0 pins, no remote font fetch in source or `dist/` | — |
| u7 | rev-m1u7-1 | U7-02 | pass | report U7-02; clean build carries exactly 6 woff2 = 176732 B, no other font format | — |
| u7 | rev-m1u7-1 | U7-03 | pass | report U7-03; six faces omit the `Variable` suffix; three role tokens keep named system + generic stacks | — |
| u7 | rev-m1u7-1 | U7-04 | pass | report U7-04; each face declares `swap` and one of the paired latin/latin-ext ranges, no other subset source | — |
| u7 | rev-m1u7-1 | U7-05 | fixed(low) | D2 promised `--field` and the rewrite retired it instead; fields consume `--surface-raised` (`src/app.css:94-109`, `QuestionCombobox.svelte:258`) | D2 rewritten to record the retirement and the one-role-one-token closure `contrast:check` grades; T1-T2's evidence row amended. The code was right and the decision text was stale, so the fix is the text |
| u7 | rev-m1u7-1 | U7-06 | pass | report U7-06; parses `:root`, WCAG sRGB luminance + ratio, floors, grades a declared table | — |
| u7 | rev-m1u7-1 | U7-07 | pass | report U7-07; 17 pairs pass category thresholds; independent sample `--border` on `--surface` = 3.41:1 floored | — |
| u7 | rev-m1u7-1 | U7-08 | pass | report U7-08; undefined, malformed, empty-table and orphan-token mutants each rc 1 with a cause-specific diagnostic, restored byte-exactly | — |
| u7 | rev-m1u7-1 | U7-09 | pass | report U7-09; `--text-muted` mutant rc 1 naming all three failing pairs; restore returns 17 passes and the original hash | — |
| u7 | rev-m1u7-1 | U7-10 | pass | report U7-10; h1 byte-equals D7; visible-copy census finds no milestone, unit, TODO, filename or catalog-id leak | — |
| u7 | rev-m1u7-1 | U7-11 | pass | report U7-11; lede claims current Prolog execution and live answers only; no trace, provenance, graph or network claim | — |
| u7 | rev-m1u7-1 | U7-12 | pass | report U7-12; prose centralized in two copy records; validator covers both plus `describe.ts` at distinct limits, 1.1 preserved | — |
| u7 | rev-m1u7-1 | U7-13 | pass | report U7-13; each banned-filler mutant fails by name; a 19-word-plus-interpolation sentence passes at the 20-word limit | — |
| u7 | rev-m1u7-1 | U7-14 | pass | report U7-14; separate 30-word `INSTRUCTIONS` and `DESCRIPTIONS` mutants each rc 1 naming file, record key and count | — |
| u7 | rev-m1u7-1 | U7-15 | pass | report U7-15; jsdom probe renders every projected descriptor humanized, keeps raw fallback, byte-matches six option labels to the generated catalog | — |
| u7 | rev-m1u7-1 | U7-16 | pass | report U7-16; always-visible footer carries attribution, nonendorsement and free availability outside the closed About disclosure | — |
| u7 | rev-m1u7-1 | U7-17 | pass | report U7-17; About states projection, universal `unreviewed` with the exact five-value vocabulary, fixed questions, no clinical use | — |
| u7 | rev-m1u7-1 | U7-18 | pass | report U7-18; three shipped OFL files byte-equal the package licences; every disclosure href resolves in `dist/` | — |
| u7 | rev-m1u7-1 | U7-19 | fixed(low) | the 11-state walker predates u7's later presentation diff and was never rerun, so overflow at the review target was unmeasured | `pnpm browser:check` gained a 320 px leg measuring five states — idle, listbox open, answers rendered, canonical open, About open — against `documentElement` scroll width and every box's right edge. Negative control: `.summary { min-width: 500px }` gives rc 1 `320px idle: document scrolls to 516px in a 320px viewport`, restored byte-exactly |
| u7 | rev-m1u7-1 | U7-20 | fixed(med) | `.box`, `.list li`, `.status`, `.alert` carried no `overflow-wrap`; only three `AnswerPanel` selectors did, so engine-authored tokens could overflow at 320 px (`QuestionCombobox.svelte:252-303`, `RunControls.svelte:89-104`) | all four now carry `overflow-wrap: anywhere`, and `tools/presentation-check.mjs` grades a declared 7-selector containment table in `pnpm gate`. Two mutants (field, status) RED; the U7-19 browser leg measures the same claim in layout |
| u7 | rev-m1u7-1 | U7-21 | pass | report U7-21; the declared gate ran from absent generated assets at rc 0 | — |
| u7 | rev-m1u7-1 | U7-22 | pass | report U7-22; the one authorized browser smoke opens the canonical disclosure and matches the rendered two-row answer to bytes read from the verified bag | — |
| u7 | rev-m1u7-1 | U7-23 | fixed(low) | contract recorded 41 walker `svelte-check` errors where 64 reproduce and `.agent/polish.md` already said 64; the walker polish entry carried no `pri` | contract G4-G5 now reads 64. `pri` added to EVERY register entry, not just the walker's, because no entry carried one — the gap was register-wide and the u7 row was one instance. Header states the high/med/low rule |
| u7 | rev-m1u7-1 | U7-24 | fixed(med) | four durable numbers contradicted the tree: copy 81 not 79, `svelte-check` 361 not 355, tests 221 not 171, `dist/` bytes drifted | refreshed to M1's closing reading under a clean-cache `pnpm gate` rc 0 — 81 strings, 366 files 0 errors 0 warnings, 228 tests in 16 files, 3 assets — and labelled as M1's close, not u7's. The reviewer measured at base `aa8dd21` (361/221); `engine:check` and `presentation:check` have since landed. `dist/`'s exact raw total is retired from memory in favour of ≈3.77 MB plus `du -sb dist`, because it re-stales on any source byte |
| u7 | rev-m1u7-1 | U7-25 | fixed(low) | D2 promised a `--field` it never defines; D1's rejected-vendoring reason cited manual subsetting, but the six package files are already latin/latin-ext subsets | both decisions rewritten. D1's rejection now rests on the real costs — a binary read-exclusion entry, a provenance record per blob, a licence-equality story — none of which beats an exact pin. Seven other decisions confirmed unchanged |
| u7 | rev-m1u7-1 | U7-26 | fixed(med) | the gate left font pins/assets/ranges/swap, OFL byte equality, D7 and framing copy, descriptor rendering and responsive containment mechanically unowned | three of five families closed by `tools/presentation-check.mjs` in `pnpm gate` — exact pins, six declared faces with `swap` + `unicode-range` + an installed local file, no remote url, three byte-equal OFL texts, seven contained surfaces — proven by 10 mutants, 10/10 RED. The two behavioural families (wordmark and forbidden-claim literals in `copy:check`, descriptor rendering in a dom test, plus fallback stacks) are filed to `.agent/polish.md` `pri` med with their acceptance check |
| cross | rev-m1-cross-1 | X01 | pass | report X01; 21 modules, 43 relative edges, zero cycles and zero upward edges; `main.ts:3` imports only `App`, which owns the demo seam | — |
| cross | rev-m1-cross-1 | X02 | accepted(low) | `tools/kb/check.mjs:21` scans sources for the answer-oracle literal but not the runtime-loaded `kb/generated` production assets; current generated bytes are clean and `kb:reproduce` bounds the blast radius | filed to `.agent/polish.md` `pri` med with the reviewer's control: plant the literal in a generated file, require `kb:asset-check` rc 1, restore, require rc 0. Not closed here — the scan's root set is u2 surface and the fix wants its own negative control |
| cross | rev-m1-cross-1 | X03 | accepted(low) | all 12 protocol arms clone cleanly and `src/engine/protocol.ts:79` declares each once, but `tests/engine-session.test.ts:279` structured-clones exactly one query response | filed to `.agent/polish.md` `pri` med: a table-driven case over all 12 discriminants, deep-compared, with a non-cloneable-field mutant red in any row |
| cross | rev-m1-cross-1 | X04 | pass | report X04; one catalog id selects the displayed question and reaches `EngineClient.query` unchanged; the paired generated goal supplies the run bytes | — |
| cross | rev-m1-cross-1 | X05 | pass | report X05; six answer outcomes, each with a producer and a distinct render branch; every switch closes on an exhaustive `never` default | — |
| cross | rev-m1-cross-1 | X06 | pass | report X06; abort propagates controller → service → client → worker → session; one cancel post, listener removal and result-owned settlement all covered | — |
| cross | rev-m1-cross-1 | X07 | pass | report X07; every exported query-capable surface requires `BudgetSpec`, the UI alone calls it with the frozen `DEMO_BUDGET`, and omission fails validation | — |
| cross | rev-m1-cross-1 | X08 | fixed(low) | the gate fails closed and out-of-gate reruns stay runnable, but `.claude/rules/gate.md`'s fenced expansion omitted `copy:check` and `contrast:check` — seven steps recorded against nine shipped | closed at `60cc2d7` before the row was written: the expansion now byte-matches `package.json` at eleven steps, `engine:check` and `presentation:check` included |
| cross | rev-m1-cross-1 | X09 | pass | report X09; 14 files ≈3.77 MB, zero engine markers in the main chunk, worker and PVM separate — graded against the memory bullet `bc3576f` rewrote, which is why the census reproduces | — |
| cross | rev-m1-cross-1 | X10 | pass | report X10; node and dom projects classify all 16 committed test files exactly once; 228/228 run, and memory's 16/228 matches | — |
| cross | rev-m1-cross-1 | X11 | pass | report X11; generated outputs ignored, none tracked, slashless `node_modules`/`kb/generated` rules match real worktree symlinks | — |
| cross | rev-m1-cross-1 | X12 | fixed(low) | 8 of 9 deny entries matched a real path; `coverage/` exists nowhere in the tree, breaching the every-rule-matches-a-real-path predicate | no coverage producer is configured, so the rule was speculative — `coverage/` removed from `.gitignore` and from `permissions.deny`, leaving 8/8 real matches. A future `--coverage` run re-adds both halves together |
| cross | rev-m1-cross-1 | X13 | fixed(med) | memory's `Engine runtime (u2)` asserted an unqualified `58 tests / 330 files`, and `Scratch conventions` said "Three files" while naming four; ten exact runtime measurements name no instrument, and seven Node figures have no browser counterpart or explicit label | the two factual errors are fixed — the u2 total is labelled historical, the count reads four. The instrument-attachment half is `.agent/polish.md` `pri` med: those numbers were measured by scratch probes that no longer exist, so attaching a pointer means re-deriving them, which is a unit not a fix |
| cross | rev-m1-cross-1 | X14 | accepted(med) | `CLAUDE.md` conforms; the memory record at 493 lines / 32,882 B retained rejected-option and correction narrative (`:19,193,203,340`), review-row origin stories (`:43-71`), and duplicates package versions, the gate script and volatile totals | partly closed by this session's archive step, which moves M1's closed-review narrative out of the attached set. The prose prune is filed `pri` med — it is a judgment pass over every line of the project's most-read file, not a fix. Closed by the retirement of that record into `.claude/rules/` |
| cross | rev-m1-cross-1 | X15 | fixed(med) | the roadmap stopped at session 5's 320 rows, still called u7 and cross-cutting unenumerated, showed no check set for u1/u6/u7, and left session 5's 31-mutant and gate figures unlabelled | roadmap rewritten for M1 REVIEWED: session 6 recorded, all seven check-set paths populated, ledger total 354 reconciled, superseded snapshots labelled historical, u3's absent unit gauge stated as unavailable |
| cross | rev-m1-cross-1 | X16 | fixed(med) | `Four-query byte differential` was already closed — `tests/questions-live.test.ts:183-196` byte-compares all four exported ids through `committedResult` — and three roadmap `NOT verified` clauses had no matching open entry | the closed entry is removed, the u2 browser-smoke clause struck from the roadmap, and u5's and u7's absent red suites filed as entries with acceptance checks. Every remaining `NOT verified` clause now maps one-to-one to an open register entry |
| cross | rev-m1-cross-1 | X17 | fixed(low) | `.agent/contracts/m1u1.md` P5.5 required generated bytes in BOTH read-exclusion halves, but `kb/generated` is gitignored, so Serena's `git_ignore=true` already excludes it — following the contract would add redundant `ignored_paths` state | P5.5 rewritten to partition: gitignored generated paths into `permissions.deny` alone, tracked non-gitignored paths into both halves, LSP-hostile readable paths into `ignored_paths` alone. Current settings satisfy the rewritten predicate |
| cross | rev-m1-cross-1 | X18 | fixed(low) | all 42 M1-era subjects state cause and fix, but seven omit the `(M1…)` trace, so the roadmap's documented grep returns 35 of 42 | history is immutable and rewriting seven commits to fix a grep is not worth the SHA churn, so the authoritative query becomes the inclusive range `git log --oneline 79233c3^..main`, recorded in the roadmap beside the trace grep with its 35/42 limit stated |
| cross | rev-m1-cross-1 | X19 | accepted(low) | `tools/smoke.mjs:53-109` and `tools/browser-check.mjs:173-279` each repeat the temp-root, `cp dist`, launch, `pageerror` and teardown sequence, despite `tools/browser.mjs` already owning `serve`, `launch` and `failWith` | filed `pri` low with the reviewer's acceptance check — one `withBuiltSite` helper, each script left with its scenario, both success paths and both negative controls preserved. Real but cosmetic: the duplication is a fixed prologue, not logic that can diverge silently |
| cross | rev-m1-cross-1 | X20 | accepted(low) | ten file headers restate their module's purpose, and eight comment sites carry review-row provenance rather than a timeless constraint | filed `pri` low. The rule it breaches is real, but a comment pass over `tools/` and `src/` is a sweep, and this session's own new headers are in scope for it |
| cross | rev-m1-cross-1 | X21 | fixed(med) | the root tree carries no `README*`, no docs directory and no reader entry document; the only Markdown outside `.agent/` is `CLAUDE.md`, which addresses agents | `README.md` written to the project's human-facing register — scope, what the demo does and does not claim, the run path, and the limits. Closed together with X23, which asks the same artifact for prerequisites and commands |
| cross | rev-m1-cross-1 | X22 | fixed(low) | `cytoscape ^3.34.2` is a production dependency with no import in `src`, `tools` or `tests`; M3 is unplanned, so it is installed for future scope alone | removed from `package.json` and the lockfile; the gate reruns rc 0. It reinstalls when M3 starts, which is when its version should be chosen anyway |
| cross | rev-m1-cross-1 | X23 | fixed(med) | a clean clone has no committed setup path — no Node or package-manager prerequisite, no `packageManager` field, no install or run command; execution cannot begin from committed state | `README.md` states the prerequisites and the ordered path, and `package.json` gains `packageManager` and `engines`. Verified from a `git clone` of `0855b4a` with no `node_modules`, running only the README's commands: node v20.19.2 (satisfies `engines`), pnpm 10.34.5 (matches `packageManager`), `install --frozen-lockfile` rc 0 lockfile-up-to-date 216 packages, `kb:build` rc 0 337 documents, `build` rc 0 131 modules |
| cross | rev-m1-cross-1 | X24 | accepted(med) | five browser claims rest on `tools/probe-u3.mjs` at `wt/rev-m1u3-4` `48008d3` and two research probes on `res-m1-*` branches, none of them ported, so deleting a branch makes seven claim families non-rerunnable from committed state | filed `pri` med: one typed browser harness covering R38/R39/R41/R42/R45, and either port the `res-m1-*` probes with their commands or prune every durable claim resting on them. The branches survive, so evidence is recoverable today and the risk is deletion, not loss |
