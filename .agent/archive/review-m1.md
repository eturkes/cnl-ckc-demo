...
| cross | rev-m1-cross-1 | X08 | fixed(low) | the gate fails closed and out-of-gate reruns stay runnable, but `.agent/memory.md`'s fenced expansion omitted `copy:check` and `contrast:check` — seven steps recorded against nine shipped | closed at `60cc2d7` before the row was written: the expansion now byte-matches `package.json` at eleven steps, `engine:check` and `presentation:check` included |
...
| cross | rev-m1-cross-1 | X14 | accepted(med) | `CLAUDE.md` conforms; `.agent/memory.md` at 493 lines / 32,882 B retains rejected-option and correction narrative (`:19,193,203,340`), review-row origin stories (`:43-71`), and duplicates package versions, the gate script and volatile totals | partly closed by this session's archive step, which moves M1's closed-review narrative out of the attached set. The prose prune is filed `pri` med — it is a judgment pass over every line of the project's most-read file, not a fix |
