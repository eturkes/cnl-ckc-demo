...
- `.agent/contracts/` = committed. Anything a later mode dispatches from lands there —
  acceptance contracts, fixed check sets, verdict tables.
...
- Surviving teammate tips stay in **branch** form (`wt/<name>`): every committed citation —
  contracts, ledgers, reviewer reports, polish entries — names the branch. A Close-order
...
Scratch-local encoding → port scheduled in `.agent/polish.md`.

A MILESTONE-REVIEW ledger is hand-maintained after its first shape; the committed ledger is
the source, and a generator is re-derived only for a bulk reshape.
